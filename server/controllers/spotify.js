...
router.get("/login", (req, res) => {
  ...
  const state = req.session.id; // Use session ID as the state parameter
...
router.get("/callback", async (req, res) => {
  ...
  if (state === null || state !== req.session.id) {
...
const fetchWithRefresh = async (url, access_token, refresh_token) => {
  try {
...
router.get("/top-artists", async (req, res) => {
  ...
  try {
    ...
    const data = await fetchWithRefresh(url, access_token, refresh_token);
...
router.get("/top-genres", async (req, res) => {
  ...
  try {
    ...
    if (!topArtistsData) {
      const url = "https://api.spotify.com/v1/me/top/artists";
      topArtistsData = await fetchWithRefresh(url, access_token, refresh_token);
